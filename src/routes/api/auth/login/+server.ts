...
import { loginWithPocketBase } from '$lib/server/backend/pocketbase';
...
export const POST: RequestHandler = async ({ request, cookies }) => {
	...
	if (config.provider === 'pocketbase') {
		...
		const result = await loginWithPocketBase(pb, email, password);
	...
	return json({ success: true, user: normalizeSupabaseUser(data.user) });
};
